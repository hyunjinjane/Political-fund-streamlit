...
from donation_program_pipeline import run_donation_pipeline
...
def render_program_tab():
    st.subheader("프로그램 입력용")
    ...
    if uploaded_file:
        ...
        with col1:
            if st.button("수입등록파일 생성하기", key="program_make_income"):
        ...
        with col2:
            if st.button("지출등록파일 생성하기", key="program_make_expense"):
                output_path = os.path.join(
                    OUTPUT_DIR,
    ...
    with config_top_col2:
        if st.button("기본 설정으로 초기화", key="reset_expense_config"):
            st.session_state["expense_config_data"] = DEFAULT_EXPENSE_CONFIG
    ...
    config_tab1, config_tab2, config_tab3 = st.tabs(
        ["지출대상자 정보", "거래기록사항 규칙", "기본 fallback 설정"]
    )
    ...
    with col3:
        if st.button("현재 설정 적용", key="apply_expense_config"):
            current_targets = st.session_state.get(
                "edited_targets_records",
                targets_df.fillna("").to_dict(orient="records")
            )
            current_rules = st.session_state.get(
                "edited_rules_records",
                rules_df.fillna("").to_dict(orient="records")
            )
            current_fallback = st.session_state.get(
                "edited_fallback_record",
                fallback_df.fillna("").to_dict(orient="records")[0]
            )
